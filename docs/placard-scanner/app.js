...
(function () {
  ...
  const unNumberInput = $('#unNumberInput');
  ...
  const notFoundText = $('#notFoundText');
  const substanceCard = $('#substanceCard');
  const pictogramList = $('#pictogramList');
  const sdsLinks = $('#sdsLinks');
  ...
  // ── Theme Toggle ──
  function initTheme() {
    const saved = localStorage.getItem('placard-theme');
    if (saved) {
  ...
  async function processImage(imageSource) {
    ...
    try {
      ...
      if (numbers && numbers.length > 0) {
        ...
        if (!found) {
          // Show first detected number even if not in database
  ...
  function manualLookup() {
    const num = unNumberInput.value.trim();
    if (!/^\d{4}$/.test(num)) {
  ...
  function displayResults(unNumber) {
    const substance = UN_NUMBERS[unNumber];
    const hazardClass = HAZARD_CLASSES[substance.class];

    // Collect all applicable pictograms
    const pictogramKeys = new Set();
    if (hazardClass) pictogramKeys.add(hazardClass.icon);
    if (substance.secondaryHazard) {
      substance.secondaryHazard.forEach(h => {
        const hc = HAZARD_CLASSES[h];
        if (hc) pictogramKeys.add(hc.icon);
      });
    }
    ...
    // Substance card
    const secondaryBadges = (substance.secondaryHazard || []).map(h => {
      const hc = HAZARD_CLASSES[h];
      return hc ? `<span class="hazard-class-badge" style="background:${hc.color};color:${getContrastColor(hc.color)}">${hc.name}</span>` : '';
    }).join('');
    ...
    substanceCard.innerHTML = `
      <div class="substance-header">
        <div>
          ...
          ${secondaryBadges}
          ${substance.guide ? `<span class="erg-guide-badge">ERG Guide ${substance.guide}</span>` : ''}
        </div>
    ...
    pictogramKeys.forEach(key => {
      const picto = GHS_PICTOGRAMS[key];
      if (!picto) return;

      const precautionHtml = picto.precautions.map(p =>
    ...
    // SDS links
    const sdsUrlList = getSDSUrls(substance);
    sdsLinks.innerHTML = sdsUrlList.map(link => `
  ...
  function buildDetailsText() {
    ...
    const { unNumber, substance, hazardClass, pictogramKeys } = currentResult;

    let text = `HAZMAT PLACARD DETAILS\n`;
    text += `${'='.repeat(40)}\n\n`;
    ...
    pictogramKeys.forEach(key => {
      ...
      if (picto) {
        ...
        text += `Precautions:\n`;
        picto.precautions.forEach(p => { text += `  - ${p}\n`; });
      }
    });
  ...
  function sendEmail() {
    ...
    const subject = encodeURIComponent(`Hazmat Placard Info - UN ${currentResult.unNumber} - ${currentResult.substance.name}`);
    const body = encodeURIComponent(buildDetailsText());
  ...
  unNumberInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') manualLookup();
  });

  // Only allow digits in the UN number input
  unNumberInput.addEventListener('input', () => {
